...
<head>
  ...
  <style>
    ...
    /* Override navbar text color for profile menu to use proper theme colors */
    #profileMenu,
    #profileMenu * {
      color: var(--ink) !important;
    }

    /* Ensure profile menu has proper background */
    #profileMenu {
    ...
    .btn-soft{ background:var(--card-bg); border:1px solid var(--card-border); color:var(--ink) } .btn-soft:hover{ background:rgba(31,41,55,.92) }
    ...
    .sidebar-item:hover{ background:rgba(31,41,55,.92) }
    ...
    .toast-card{ background:var(--card-bg); border:1px solid var(--card-border); border-radius:.75rem; padding:.6rem .9rem; box-shadow:0 10px 30px rgba(0,0,0,.08); color:var(--ink) }
    ...
    #notificationPanel .notification-link {
      color: #2563eb;
    }
    
    #notificationPanel .notification-link:hover {
      color: #1d4ed8;
    }
...
<body class="min-h-screen text-[15px] bg-soft">
  ...
  <header class="sticky top-0 z-50 border-b border-[var(--ring)] navbar backdrop-blur">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-14 flex items-center gap-3">
      ...
      <div class="relative">
        <button id="profileBtn" type="button" class="p-2 rounded hover:bg-white/10 flex items-center gap-2" title="Account">
          <img src="<?php echo htmlspecialchars($user['profile_image'] ?? 'uploads/admindefault.png'); ?>"
        ...
        <div id="profileMenu" class="hidden absolute right-0 mt-2 w-56 bg-[var(--card-bg)] rounded-lg shadow-xl border border-[var(--card-border)] overflow-hidden text-[var(--ink)]">
          ...
          <div class="px-4 py-2 border-b border-[var(--card-border)]">
            ...
            <div class="text-xs text-[var(--muted)]"><?php echo htmlspecialchars($user['role_name'] ?? ''); ?></div>
          </div>
          <a href="settings.php" class="block px-4 py-2 text-[var(--ink)] hover:bg-[var(--card-bg)]">Settings</a>
          <a href="profile.php" class="block px-4 py-2 text-[var(--ink)] hover:bg-[var(--card-bg)]">Profile</a>
          <a href="logout.php" class="block px-4 py-2 text-[var(--ink)] hover:bg-[var(--card-bg)]">Logout</a>
        </div>
      </div>
    </div>
  </header>
    ...
    <div id="overlay" class="overlay"></div>
      ...
      <nav id="sidebarNav" class="p-3 space-y-1">
        ...
        <a class="sidebar-item" href="Reports.php"><span>📑</span><span>Reports</span></a>
         ...
         <main class="space-y-6 overflow-y-auto min-h-[calc(100vh-8rem)] pr-2">
      <section id="dashboardHome" class="space-y-6">
         <!-- Secondary Navigation Bar -->
         <div class="mb-6">
           <div class="flex items-center justify-between">
             <h1 class="text-2xl md:text-3xl font-bold" style="color: var(--ink)">Financial Dashboard</h1>
           </div>
         </div>
         ...
         <div class="mb-6">
           <div class="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
             <div>
               <div class="text-sm md:text-base font-medium mb-1" style="color: var(--muted)">FINANCIAL DASHBOARD</div>
               <h2 class="text-2xl md:text-3xl font-bold" style="color: var(--ink)">Overview</h2>
             </div>
             <div class="btn-group flex flex-col sm:flex-row gap-3">
                               <button id="refreshDashboard" class="btn btn-brand flex items-center gap-2 transition-all duration-300 hover:scale-105 px-4 py-2 min-h-[44px]">
               ...
               <button id="exportDashboard" class="btn btn-soft flex items-center gap-2 px-4 py-2 min-h-[44px]">
         ...
         <div class="card p-4 mb-6">
           <div class="flex items-center justify-between">
             <div class="flex items-center gap-4">
               <span class="text-sm" style="color: var(--muted)">Last updated: <span id="lastUpdated" class="font-semibold" style="color: var(--ink)"><?php echo date('M d, Y H:i:s'); ?></span></span>
             </div>
        ...
        <div class="card p-6">
          <h3 class="text-xl md:text-2xl font-bold mb-6">Quick Actions</h3>
          <div class="quick-actions-grid">
            <button onclick="window.location.href='General Ledger.php'" class="p-6 border border-slate-200 rounded-xl hover:bg-slate-50 transition-all duration-300 hover:scale-105 hover:shadow-lg text-left min-h-[120px] group">
              ...
              <div class="font-bold text-lg mb-2">New Journal Entry</div>
              <div class="text-base text-slate-500">Record transactions</div>
            </button>

            <button onclick="window.location.href='Accounts Receivable.php'" class="p-6 border border-slate-200 rounded-xl hover:bg-slate-50 transition-all duration-300 hover:scale-105 hover:shadow-lg text-left min-h-[120px] group">
              ...
              <div class="font-bold text-lg mb-2">Record Receivable</div>
              <div class="text-base text-slate-500">Track money owed</div>
            </button>

            <button onclick="window.location.href='Accounts Payable.php'" class="p-6 border border-slate-200 rounded-xl hover:bg-slate-50 transition-all duration-300 hover:scale-105 hover:shadow-lg text-left min-h-[120px] group">
              ...
              <div class="font-bold text-lg mb-2">Record Payable</div>
              <div class="text-base text-slate-500">Track money owed</div>
            </button>

            <button onclick="window.location.href='Reports.php'" class="p-6 border border-slate-200 rounded-xl hover:bg-slate-50 transition-all duration-300 hover:scale-105 hover:shadow-lg text-left min-h-[120px] group">
              ...
              <div class="font-bold text-lg mb-2">Generate Reports</div>
              <div class="text-base text-slate-500">Financial analysis</div>
            </button>
          </div>
        </div>
        
                 <div id="kpiGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
           <div class="card p-5 relative overflow-hidden dashboard-card" data-title="cash">
             <div class="absolute inset-x-0 top-0 top-chip"></div>
             <div class="text-sm uppercase tracking-wider font-semibold mb-2" style="color: var(--muted)">Cash Balance</div>
             <div class="text-2xl md:text-3xl font-bold mt-2 mb-3" id="cashBalanceValue"><?php echo formatCurrency($data['cash_balance']); ?></div>
             <div class="text-sm font-medium mb-4" style="color: var(--muted)">Current Balance</div>
             <button class="w-full btn btn-brand hover:scale-105 transition-transform duration-200 px-4 py-3 min-h-[48px]" onclick="window.location.href='General Ledger.php'">View Ledger ▸</button>
           </div>
           <div class="card p-5 relative overflow-hidden dashboard-card" data-title="ar outstanding">
             <div class="absolute inset-x-0 top-0 top-chip"></div>
             <div class="text-sm uppercase tracking-wider font-semibold mb-2" style="color: var(--muted)">AR Outstanding</div>
             <div class="text-2xl md:text-3xl font-bold mt-2 mb-3" id="arOutstandingValue"><?php echo formatCurrency($data['ar_outstanding']); ?></div>
             <div class="text-sm font-medium mb-4" style="color: var(--muted)">Accounts Receivable</div>
             <button class="w-full btn btn-brand hover:scale-105 transition-transform duration-200 px-4 py-3 min-h-[48px]" onclick="window.location.href='Accounts Receivable.php'">View AR ▸</button>
           </div>
           <div class="card p-5 relative overflow-hidden dashboard-card" data-title="ap outstanding">
             <div class="absolute inset-x-0 top-0 top-chip"></div>
             <div class="text-sm uppercase tracking-wider font-semibold mb-2" style="color: var(--muted)">AP Outstanding</div>
             <div class="text-2xl md:text-3xl font-bold mt-2 mb-3" id="apOutstandingValue"><?php echo formatCurrency($data['ap_outstanding']); ?></div>
             <div class="text-sm font-medium mb-4" style="color: var(--muted)">Accounts Payable</div>
             <button class="w-full btn btn-brand hover:scale-105 transition-transform duration-200 px-4 py-3 min-h-[48px]" onclick="window.location.href='Accounts Payable.php'">View AP ▸</button>
           </div>
           <div class="card p-5 relative overflow-hidden dashboard-card" data-title="revenue mtd">
             <div class="absolute inset-x-0 top-0 top-chip"></div>
             <div class="text-sm uppercase tracking-wider font-semibold mb-2" style="color: var(--muted)">Revenue (MTD)</div>
             <div class="text-2xl md:text-3xl font-bold mt-2 mb-3" id="revenueMTDValue"><?php echo formatCurrency($data['revenue_mtd']); ?></div>
             <div class="text-sm font-medium mb-4" style="color: var(--muted)"><?php echo $currentMonthName; ?></div>
             <button class="w-full btn btn-brand hover:scale-105 transition-transform duration-200 px-4 py-3 min-h-[48px]" onclick="window.location.href='Reports.php'">View Reports ▸</button>
           </div>
         </div>
        ...
        <div class="monthly-summary-grid mb-6">
          <div class="card p-4 text-center">
            <div class="text-sm uppercase tracking-wider font-semibold mb-2" style="color: var(--muted)">Total Revenue</div>
            <div class="text-xl md:text-2xl font-bold text-green-600 mb-2"><?php echo formatCurrency($data['monthly_summary']['total_revenue']); ?></div>
            <div class="text-sm font-medium" style="color: var(--muted)"><?php echo $currentMonthName; ?></div>
          </div>
          <div class="card p-4 text-center">
            <div class="text-sm uppercase tracking-wider font-semibold mb-2" style="color: var(--muted)">Total Expenses</div>
            <div class="text-xl md:text-2xl font-bold text-red-600 mb-2"><?php echo formatCurrency($data['monthly_summary']['total_expenses']); ?></div>
            <div class="text-sm font-medium" style="color: var(--muted)"><?php echo $currentMonthName; ?></div>
          </div>
          <div class="card p-4 text-center">
            <div class="text-sm uppercase tracking-wider font-semibold mb-2" style="color: var(--muted)">Net Income</div>
            <div class="text-xl md:text-2xl font-bold <?php echo $data['monthly_summary']['net_income'] >= 0 ? 'text-green-600' : 'text-red-600'; ?> mb-2">
        ...
        <div class="chart-table-layout">
             ...
             <div class="px-6 pt-5 pb-3 flex items-center justify-between">
               ...
               <div class="flex gap-2">
                 <button class="tab-pill active text-sm px-4 py-2 min-h-[40px]" data-period="7d">7D</button>
                 <button class="tab-pill text-sm px-4 py-2 min-h-[40px]" data-period="30d">30D</button>
                 <button class="tab-pill text-sm px-4 py-2 min-h-[40px]" data-period="90d">90D</button>
               </div>
             </div>
             <div class="p-6 pt-0 relative">
               <div id="chartLoader" class="absolute inset-0 flex items-center justify-center bg-white/80 backdrop-blur-sm z-10 rounded-lg">
                 <div class="flex flex-col items-center gap-4">
                   <div class="w-10 h-10 border-4 border-blue-200 border-t-blue-500 rounded-full animate-spin"></div>
                   <p class="text-base text-slate-600 font-medium">Loading chart...</p>
                 </div>
               </div>
          ...
          <div class="card p-0 overflow-hidden">
            <div class="px-6 pt-5 pb-3 flex items-center justify-between">
              ...
              <a href="#" class="text-base text-blue-600 hover:underline font-semibold">View All</a>
            </div>
              ...
              <table class="w-full text-base">
                <thead class="bg-slate-50">
                  <tr>
                    ...
                    <th class="text-left p-4 font-semibold text-slate-700">Description</th>
                    <th class="text-left p-4 font-semibold text-slate-700">Type</th>
                    <th class="text-right p-4 font-semibold text-slate-700">Amount</th>
                  </tr>
                </thead>
                <tbody>
                  <?php if (!empty($data['recent_transactions'])): ?>
                    <?php foreach ($data['recent_transactions'] as $transaction): ?>
                      <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                        <td class="p-4 text-slate-600"><?php echo date('M d, Y', strtotime($transaction['date'])); ?></td>
                        <td class="p-4 text-slate-800 font-medium"><?php echo htmlspecialchars($transaction['description'] ?? ''); ?></td>
                        <td class="p-4">
                          <?php if ($transaction['type'] === 'Income'): ?>
                            <span class="px-3 py-1.5 bg-green-100 text-green-800 rounded-full text-sm font-medium">Income</span>
                          <?php elseif ($transaction['type'] === 'Expense'): ?>
                            <span class="px-3 py-1.5 bg-red-100 text-red-800 rounded-full text-sm font-medium">Expense</span>
                          <?php else: ?>
                            <span class="px-3 py-1.5 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">Transaction</span>
                          <?php endif; ?>
                        </td>
                        <td class="p-4 text-right font-bold text-lg">
                          ...
                          $colorClass = ($transaction['type'] === 'Income') ? 'text-green-600' : (($transaction['type'] === 'Expense') ? 'text-red-600' : 'text-slate-800');
                          echo '<span class="' . $colorClass . '">' . $prefix . formatCurrency($amount) . '</span>';
                          ?>
                        </td>
                      </tr>
                    <?php endforeach; ?>
                  <?php else: ?>
                    <tr>
                      <td colspan="4" class="p-6 text-center text-slate-500 text-lg">No recent transactions found</td>
                    </tr>
                  <?php endif; ?>
                </tbody>
              </table>
    ...
    const dashboardData = {
      ...
      revenueMTD: <?php echo $data['revenue_mtd']; ?>
       ...
       $('#liveTimeMobile').textContent = timeStr;
         ...
         function initCharts() {
       // Initialize with default 7-day period
       updateSalesChart('7d');
     }
      ...
      if (profileBtn && profileMenu) {
        ...
        if (profileImg) {
          profileImg.addEventListener('click', (e) => {
            ...
            e.stopPropagation();
            profileMenu.classList.toggle('hidden');
            console.log('Profile dropdown toggled via image');
            return false;
          });
        }

        // Handle menu item clicks - ensure links work properly
   ...
   function initDashboardRefresh() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
     ...
     function initDashboardTabs() {
       const tabButtons = document.querySelectorAll('[data-period]');
     ...
     function updateSalesChart(period) {
       ...
       const data = filteredData.map(item => item.sales).reverse();
         ...
         window.salesChartInstance = new Chart(ctx, {
           ...
           data: {
             ...
             datasets: [{
               ...
               fill: true,
               tension: 0.4,
               pointBackgroundColor: '#0f1c49',
               pointBorderColor: '#ffffff',
               pointBorderWidth: 2,
               pointRadius: 5,
               pointHoverRadius: 7,
               pointHoverBackgroundColor: '#0f1c49',
           ...
           options: {
             ...
             plugins: {
               ...
               tooltip: {
                 backgroundColor: 'rgba(15, 28, 73, 0.95)',
                 titleColor: '#ffffff',
                 bodyColor: '#ffffff',
      ...
      async function generateDashboardPDF() {
        ...
        // Add logo watermark (centered, 0.5 opacity)
        try {
          // Load logo image
          ...
          // Calculate logo dimensions (maintain aspect ratio, max 80mm wide)
          const logoMaxWidth = 80;
          const logoAspectRatio = logoImg.width / logoImg.height;
          const logoWidth = Math.min(logoMaxWidth, logoImg.width);
          ...
          // Add logo with 0.2 opacity as watermark (stamp-like appearance)
        ...
        const now = new Date();
        pdf.text(`Generated on: ${now.toLocaleDateString()} ${now.toLocaleTimeString()}`, margin, currentY);
        currentY += 10;

        // Add current month
        pdf.text(`Report Period: ${dashboardData.currentMonth}`, margin, currentY);
        currentY += 20;
        ...
        const kpis = [
          { label: 'Cash Balance', value: dashboardData.cashBalance },
          { label: 'AR Outstanding', value: dashboardData.arOutstanding },
          { label: 'AP Outstanding', value: dashboardData.apOutstanding },
          { label: 'Revenue (MTD)', value: dashboardData.revenueMTD }
        ];
        ...
        kpis.forEach(kpi => {
          ...
          pdf.text(`${kpi.label}: PHP ${kpi.value.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, margin, currentY);
          currentY += 8;
        });
        ...
        pdf.setFontSize(16);
        pdf.setFont('helvetica', 'bold');
        pdf.text('Monthly Summary', margin, currentY);
        currentY += 15;

        pdf.setFontSize(12);
        pdf.setFont('helvetica', 'normal');

        // Get monthly summary data from PHP
        const monthlyData = <?php echo json_encode($data['monthly_summary']); ?>;

        const monthlyItems = [
          { label: 'Total Revenue', value: monthlyData.total_revenue },
          { label: 'Total Expenses', value: monthlyData.total_expenses },
          { label: 'Net Income', value: monthlyData.net_income }
        ];
        ...
        monthlyItems.forEach(item => {
          ...
          pdf.text(`${item.label}: PHP ${item.value.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, margin, currentY);
          currentY += 8;
        });
        ...
        pdf.setFontSize(16);
        pdf.setFont('helvetica', 'bold');
        pdf.text('Recent Transactions', margin, currentY);
        currentY += 15;

        // Get transactions data from PHP
        const transactions = <?php echo json_encode($data['recent_transactions']); ?>;
        ...
        if (transactions && transactions.length > 0) {
          ...
          // Table rows
          pdf.setFont('helvetica', 'normal');
          transactions.slice(0, 10).forEach(transaction => { // Limit to 10 transactions
            ...
            const amount = (transaction.entry_type === 'debit' && transaction.type === 'Expense' ? '-' : '') +
                          'PHP ' + parseFloat(transaction.amount).toLocaleString('en-US', {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2
                          });
            ...
            const rowData = [
              date,
              transaction.description ? transaction.description.substring(0, 25) + (transaction.description.length > 25 ? '...' : '') : '',
          ...
          pdf.setFontSize(12);
          pdf.text('No recent transactions found.', margin, currentY);
        }
        ...
        pdf.setFontSize(16);
        pdf.setFont('helvetica', 'bold');
        pdf.text('Sales Trend Summary', margin, currentY);
        currentY += 15;
        ...
        if (salesData && salesData.length > 0) {
          ...
          const maxSales = Math.max(...salesData.map(item => parseFloat(item.sales)));
          const minSales = Math.min(...salesData.map(item => parseFloat(item.sales)));

          const stats = [
            `Total Sales: PHP ${totalSales.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
            `Average Daily Sales: PHP ${avgSales.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
            `Highest Daily Sales: PHP ${maxSales.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
            `Lowest Daily Sales: PHP ${minSales.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
          ];
        ...
        for (let i = 1; i <= totalPages; i++) {
          ...
          pdf.text('Generated by ATIERA Financial Suite', margin, pageHeight - 10);
        }

        // Save the PDF
        const fileName = `dashboard-report-${now.getFullYear()}${(now.getMonth()+1).toString().padStart(2,'0')}${now.getDate().toString().padStart(2,'0')}.pdf`;
        pdf.save(fileName);
      }
