...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['username'])) {
        ...
        if (empty($newUsername)) {
            ...
            $result = $auth->updateUsername($user['id'], $newUsername);
            if ($result['success']) {
    ...
    if (isset($_FILES['profile_image']) && $_FILES['profile_image']['error'] === UPLOAD_ERR_OK) {
        ...
        $allowedMimeTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'application/octet-stream'];
        ...
        if ($_FILES['profile_image']['size'] > $maxFileSize) {
            ...
            error_log("File validation - Name: " . $_FILES['profile_image']['name']);
            error_log("File validation - Extension: " . $fileExtension);
            error_log("File validation - MIME type: " . $fileMimeType);
            error_log("Allowed extensions: " . implode(', ', $allowedExtensions));
            error_log("Allowed MIME types: " . implode(', ', $allowedMimeTypes));

            // Validate file extension and MIME type
            if (!in_array($fileExtension, $allowedExtensions) || !in_array($fileMimeType, $allowedMimeTypes)) {
                $message = 'Invalid file type. Only JPG, PNG, and GIF are allowed.';
                $messageType = 'error';
                error_log("File validation failed - Extension: $fileExtension, MIME: $fileMimeType");
            } else {
                ...
                if (!$imageInfo) {
                    ...
                    // Sanitize filename
                    $fileName = 'profile_' . $user['id'] . '_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $fileExtension;
                    ...
                    if (move_uploaded_file($_FILES['profile_image']['tmp_name'], $uploadPath)) {
                        ...
                        if (!empty($user['profile_image']) && $user['profile_image'] !== 'admindefault.png' && file_exists($uploadDir . $user['profile_image'])) {
                            unlink($uploadDir . $user['profile_image']);
                        }

                        // Debug: Log the filename being saved
                        error_log("Saving profile image filename: " . $fileName);

                        error_log("About to call updateProfileImage with user_id: " . $user['id'] . " and path: " . $uploadPath);
                        ...
                        error_log("updateProfileImage result: " . json_encode($result));

                        // Check if the update actually worked by querying the database
                        try {
                            $db = new Database();
                            $checkUser = $db->fetchOne("SELECT profile_image FROM users WHERE id = ?", [$user['id']]);
                            error_log("Profile image in database after update: " . ($checkUser['profile_image'] ?? 'NULL'));
                        } catch (Exception $e) {
                            error_log("Failed to check database: " . $e->getMessage());
                        }
                        if ($result['success']) {
                            ...
                            error_log("Profile image updated successfully for user ID: " . $user['id']);

                            // Refresh user data
                            $user = $auth->getCurrentUser();

                            // Debug: Check if user data was refreshed
                            error_log("Refreshed user profile_image: " . ($user['profile_image'] ?? 'NULL'));

                            // Force browser to reload the new image by adding cache buster
                            $cacheBuster = '?t=' . time();
                            $newImagePath = $uploadPath . $cacheBuster;

                            // Update the preview image source immediately
                            echo "<script>
                                console.log('Updating profile image to: $newImagePath');
                                document.getElementById('profilePreview').src = '$newImagePath';
                                // Also update header profile image if it exists
                                const headerProfileImg = document.querySelector('header img[alt=\"Profile\"]');
                                if (headerProfileImg) {
                                    headerProfileImg.src = 'uploads/$fileName$cacheBuster';
                            ...
                            $message = $result['message'];
                            $messageType = 'error';
                            error_log("Failed to update profile image: " . $result['message']);
...
<head>
            ...
            <form method="POST" enctype="multipart/form-data" class="space-y-6">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <p class="text-xs text-gray-500 mt-1">Max 10MB, JPG/PNG/GIF only</p>
                  ...
                  <div class="mt-4">
                    ...
                    <div class="relative inline-block">
                      <img id="profilePreview"
                           src="<?php echo htmlspecialchars($user['profile_image'] ?? 'uploads/admindefault.png'); ?>"
                           alt="Profile Preview"
                ...
                <div>
                  ...
                  <p class="text-xs text-gray-500 mt-1">Choose a unique username (3-50 characters)</p>
                </div>
              </div>

              <div class="flex gap-3">
                <button type="submit" class="btn btn-brand">Save Changes</button>
                <button type="button" class="btn btn-soft" onclick="showSection('overview')">Cancel</button>
              </div>
            </form>
          </div>
        </div>
  ...
  <div id="cropModal" class="fixed inset-0 z-[200] hidden">
    <div class="flex items-center justify-center min-h-screen p-4">
      <div class="fixed inset-0 bg-black/50 backdrop-blur-sm" id="cropModalBackdrop"></div>
      <div class="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
        <div class="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Crop Profile Image</h3>
          <button id="closeCropModal" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
        ...
        <div class="p-4">
          <div class="flex flex-col items-center space-y-4">
            <div class="w-full max-w-md">
              <img id="cropImage" class="max-w-full h-auto" alt="Image to crop">
            </div>

            <div class="flex flex-wrap gap-2 justify-center">
              <button id="cropButton" class="btn btn-brand">Crop & Save</button>
              <button id="cancelCropButton" class="btn btn-soft">Cancel</button>
            </div>

            <div class="text-sm text-gray-600 dark:text-gray-400 text-center">
              <p>Drag to move, use corners to resize the crop area</p>
            </div>
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    // Handle direct links to edit profile
    const editProfileLinks = $$('a[href="#profile/edit"]');
    editProfileLinks.forEach(link => {
      link.addEventListener('click', (e) => {
        e.preventDefault();
        // Switch to edit tab
        tabs.forEach(t => t.classList.remove('active'));
        const editTab = $('#contextTabs .tab-pill[href="#profile/edit"]');
        if (editTab) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
          ...
          if (cropper) {
            ...
            if (canvas) {
              ...
              canvas.toBlob(function(blob) {
                ...
                sessionStorage.setItem('croppedProfileImage', JSON.stringify({
                  dataUrl: croppedDataUrl,
                  type: selectedFile.type,
                ...
                showToast('Image cropped successfully! Click "Save Changes" to upload.', 'success');

              }, selectedFile.type, 0.9); // 0.9 quality for good compression
            }
          }
        });
      }
      ...
      if (cropModalBackdrop) {
        cropModalBackdrop.addEventListener('click', function() {
          closeModal();
        });
      }
      ...
      // Load cropped image from sessionStorage on page load
      const savedCroppedImage = sessionStorage.getItem('croppedProfileImage');
      if (savedCroppedImage && profilePreview) {
        try {
          const imageData = JSON.parse(savedCroppedImage);
          profilePreview.src = imageData.dataUrl;
        } catch (e) {
          console.error('Error loading saved cropped image:', e);
        }
      }
      ...
      if (form) {
        form.addEventListener('submit', function(e) {
          const savedImage = sessionStorage.getItem('croppedProfileImage');
          if (savedImage) {
