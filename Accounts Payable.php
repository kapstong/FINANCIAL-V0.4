...
<head>
  ...
  <style>
    ...
    html.dark .bg-slate-50 { background: rgba(31,41,55,.95); }
    html.dark .bg-red-50 { background: rgba(153,27,27,.3); }
    html.dark .text-slate-700 { color: var(--muted); }
    html.dark .text-slate-600 { color: var(--muted); }
    html.dark .text-slate-500 { color: var(--muted); }
    html.dark .border-slate-200 { border-color: rgba(71,85,105,.55); }
    html.dark .border-slate-300 { border-color: rgba(71,85,105,.55); }
    
    /* Enhanced dark mode styles for Accounts Payable */
    html.dark .bg-white { background: var(--card-bg); }
    html.dark .bg-white\/80 { background: rgba(17,24,39,.8); }
    html.dark .bg-white\/95 { background: rgba(17,24,39,.95); }
    
    /* Dark mode for table elements */
    html.dark .bg-red-50 { background: rgba(239,68,68,.1); }
    html.dark .border-t { border-color: rgba(71,85,105,.3); }
    
    /* Dark mode for text colors */
    html.dark .text-black { color: var(--ink); }
    html.dark .text-slate-800 { color: var(--ink); }
    
    /* Dark mode for hover effects */
    html.dark .hover\:bg-slate-900:hover { background: rgba(31,41,55,.95); }
    
    /* Dark mode for context bar */
    ...
    html.dark .overlay.active { background: rgba(0,0,0,.6); }
    
    /* Dark mode for main content */
    html.dark main { background: transparent; }
    html.dark #contentHost { background: transparent; }
    
    /* Dark mode for table headers */
    html.dark thead.bg-red-50 { background: rgba(239,68,68,.1); }
    
    /* Dark mode for table rows */
    html.dark tbody tr.border-t { border-color: rgba(71,85,105,.3); }
    
    /* Dark mode for action links */
    html.dark a.text-slate-600 { color: var(--muted); }
    html.dark a.text-slate-600:hover { color: var(--ink); }
    
    /* Dark mode for summary cards */
    html.dark .card .font-semibold { color: var(--ink); }
    
    /* Dark mode for navigation elements */
    html.dark .sidebar-item { color: var(--ink); }
    html.dark .sidebar-item:hover { background: rgba(31,41,55,.92); }
    html.dark .sidebar-item.active { 
      background: rgba(15,28,73,.15); 
      color: var(--brand-100); 
    }
    
    /* Dark mode for context tabs */
    html.dark .tab-pill { 
      color: var(--ink); 
      border-color: rgba(71,85,105,.55); 
    }
...
<body class="min-h-screen text-[15px] text-[var(--ink)] bg-soft">
  ...
  <div id="globalLoader" class="fixed inset-0 z-[100] flex items-center justify-center bg-gradient-to-br from-slate-900/95 to-slate-800/95 backdrop-blur-sm">
    <div class="flex flex-col items-center gap-6">
      ...
      <div class="text-center">
        <h2 class="text-2xl font-bold text-white mb-2">Loading Accounts Payable</h2>
        <p class="text-slate-300 text-sm">Preparing your payables data...</p>
      </div>
  ...
  <div id="contextBar" class="sticky top-14 z-40 border-b border-[var(--ring)] bg-white/80 backdrop-blur">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-12 flex items-center justify-between">
      <div class="flex items-center gap-3">
        <span class="font-semibold" style="color: var(--ink)">Accounts Payable</span>
      </div>
      <nav id="contextTabs" class="flex flex-wrap gap-2">
        <a class="tab-pill" href="#Accounts%20Payable/ap-open">Open Bills</a>
        <a class="tab-pill" href="#Accounts%20Payable/ap-paid">Payment History</a>
        <a class="tab-pill" href="#Accounts%20Payable/ap-aging">Aging Report</a>
      </nav>
    </div>
  </div>
  ...
  <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-6 py-6">
    ...
    <aside id="sidebar" class="fixed lg:static left-0 top-14 lg:top-auto w-64 lg:w-full h-[calc(100vh-56px)] lg:h-auto bg-white border-r border-[var(--ring)] sidebar-transition -translate-x-full lg:translate-x-0 z-50 overflow-y-auto">
      <nav class="p-3 space-y-1">
        <div class="text-[11px] uppercase tracking-widest px-2 pt-2 pb-1" style="color: var(--muted)">Navigation</div>
        ...
        <a class="sidebar-item active" href="Accounts Payable.php"><span>📄</span><span>Accounts Payable</span></a>
  ...
  <script>
    ...
    const AP_TABS = [
      ...
      { id:'ap-paid',  label:'Payment History' },
      { id:'ap-aging', label:'Aging Report' },
    ];

    function decodeHash(){
      const h=(location.hash||'#Accounts%20Payable/ap-open').slice(1);
      const [mod, tab]=h.split('/');
      return { module: decodeURIComponent(mod||'Accounts Payable'), tab: decodeURIComponent(tab||'ap-open') };
    }

    function markActiveTab(tabId){
      $$('#contextTabs .tab-pill').forEach(a=>{
        a.classList.toggle('active', a.getAttribute('href') === `#Accounts%20Payable/${tabId}`);
      });
    }

    function renderView(tabId){
      const tab = AP_TABS.find(t=>t.id===tabId) || AP_TABS[0];
      const host = $('#contentHost');
      host.innerHTML = `
        <section class="card p-5">
          <div class="flex items-center justify-between">
            <div>
              <div class="text-xs uppercase tracking-wide text-[var(--muted)]">ACCOUNTS PAYABLE</div>
              <h2 class="text-xl font-bold">${tab.label}</h2>
            </div>
            <div class="flex gap-2">
              <button class="btn btn-soft">+ Add Bill</button>
          ...
          <div class="mt-4 border-t border-[var(--ring)] pt-4">
            <div class="overflow-x-auto">
              <table class="min-w-full text-sm">
                <thead class="bg-red-50" style="background: rgba(239,68,68,.1); border-color: rgba(248,113,113,.3)">
                  <tr>
                    <th class="text-left px-3 py-2" style="color: var(--ink)">Date</th>
                    <th class="text-left px-3 py-2" style="color: var(--ink)">Reference</th>
                    <th class="text-left px-3 py-2" style="color: var(--ink)">Vendor</th>
                    <th class="text-left px-3 py-2" style="color: var(--ink)">Amount</th>
                    <th class="text-left px-3 py-2" style="color: var(--ink)">Due Date</th>
                    <th class="text-left px-3 py-2" style="color: var(--ink)">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  <tr class="border-t" style="border-color: rgba(71,85,105,.3)">
                    <td class="px-3 py-2" style="color: var(--ink)">2025-08-01</td>
                    <td class="px-3 py-2" style="color: var(--ink)">BILL-0001</td>
                    <td class="px-3 py-2" style="color: var(--ink)">Sample Vendor</td>
                    <td class="px-3 py-2" style="color: var(--ink)">₱2,500.00</td>
                    <td class="px-3 py-2" style="color: var(--ink)">2025-08-15</td>
                    <td class="px-3 py-2">
                      <a href="#" class="text-[var(--brand)] font-semibold hover:underline">View</a>
                      <a href="#" class="ml-3 text-slate-600 hover:underline" style="color: var(--muted)">Edit</a>
            ...
            <div class="mt-5 grid grid-cols-1 md:grid-cols-2 gap-5">
              <div class="card p-4">
                <div class="font-semibold mb-2" style="color: var(--ink)">Payable Summary</div>
                <p class="text-sm" style="color: var(--muted)">Total outstanding: <b>₱8,750.00</b></p>
                <p class="text-sm" style="color: var(--muted)">Overdue: <b>₱1,200.00</b></p>
              </div>
              <div class="card p-4">
                <div class="font-semibold mb-2" style="color: var(--ink)">Quick Actions</div>
                <p class="text-sm" style="color: var(--muted)">Add quick action buttons for <b>${tab.label}</b> here.</p>
              </div>
            </div>
          </div>
        </section>
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      if(!location.hash){
        location.hash = '#Accounts%20Payable/ap-open';
      }
    ...
    function initNotificationSystem() {
      ...
      // Simulate real-time notifications for this module
      simulateModuleNotifications();
    }
    ...
    // Simulate module-specific notifications
    function simulateModuleNotifications() {
      // Generic notifications (every 3 minutes)
      setInterval(() => {
        if (Math.random() < 0.2) { // 20% chance
          const messages = [
            'System update completed successfully',
            'New data synchronization in progress',
            'Module performance: All systems operational',
