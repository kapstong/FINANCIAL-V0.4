...
<head>
  ...
  <style>
    ...
    /* Dark mode for context bar and sidebar */
    ...
    /* Additional responsive improvements for content */
    @media (min-width: 1024px) {
      .card {
        width: 100%;
        max-width: none;
      ...
      /* Ensure filters use full width on desktop */
      .filter-section {
        width: 100%;
      }
    ...
    <div id="globalLoader" class="fixed inset-0 z-[100] flex items-center justify-center bg-gradient-to-br from-slate-900/95 to-slate-800/95 backdrop-blur-sm">
      <div class="flex flex-col items-center gap-6">
        ...
        <div class="text-center">
          <h2 class="text-2xl font-bold text-white mb-2">Loading Collections</h2>
          <p class="text-slate-300 text-sm">Preparing your collections data...</p>
        </div>
  ...
  <div id="contextBar" class="sticky top-14 z-40 border-b border-[var(--ring)] bg-white/80 backdrop-blur">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-12 flex items-center justify-between">
      <div class="flex items-center gap-3">
        <span class="font-semibold" style="color: var(--ink)">Collections</span>
      </div>
      <nav id="contextTabs" class="flex flex-wrap gap-2">
        <a class="tab-pill" href="#Collections/col-pending">Pending Collections</a>
        <a class="tab-pill" href="#Collections/col-history">Collection History</a>
        <a class="tab-pill" href="#Collections/col-reports">Reports</a>
      </nav>
    </div>
  </div>
  ...
  <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-6 py-6">
    ...
    <aside id="sidebar" class="fixed lg:static left-0 top-14 lg:top-auto w-64 lg:w-full h-[calc(100vh-56px)] lg:h-auto bg-white border-r border-[var(--ring)] sidebar-transition -translate-x-full lg:translate-x-0 z-50 overflow-y-auto">
      <nav class="p-3 space-y-1">
        <div class="text-[11px] uppercase tracking-wide px-2 pt-2 pb-1" style="color: var(--muted)">Navigation</div>
        ...
        <a class="sidebar-item active" href="Collections.php"><span>🧾</span><span>Collections</span></a>
  ...
  <script>
    ...
    $('#darkModeToggle')?.addEventListener('click', ()=>{ 
      ...
      // Force notification panel style recalculation
      const notificationPanel = document.getElementById('notificationPanel');
      if (notificationPanel) {
        notificationPanel.style.display = 'none';
        notificationPanel.offsetHeight; // Force reflow
    ...
    /* --------- Collections tabs + view --------- */
    const COL_TABS = [
      { id:'col-pending',  label:'Pending Collections' },
      { id:'col-history',  label:'Collection History' },
      { id:'col-reports',  label:'Reports' },
    ];

    function decodeHash(){
      const h=(location.hash||'#Collections/col-pending').slice(1);
      const [mod, tab]=h.split('/');
      return { module: decodeURIComponent(mod||'Collections'), tab: decodeURIComponent(tab||'col-pending') };
    }

    function markActiveTab(tabId){
      $$('#contextTabs .tab-pill').forEach(a=>{
        a.classList.toggle('active', a.getAttribute('href') === `#Collections/${tabId}`);
      });
    }

    function renderView(tabId){
      const tab = COL_TABS.find(t=>t.id===tabId) || COL_TABS[0];
      const host = $('#contentHost');
      host.innerHTML = `
        <section class="card p-5">
          <div class="flex items-center justify-between">
            <div>
              <div class="text-xs uppercase tracking-wide text-[var(--muted)]">COLLECTIONS</div>
              <h2 class="text-xl font-bold">${tab.label}</h2>
            </div>
            <div class="flex gap-2">
              <button class="btn btn-soft">+ New Collection</button>
            ...
            <div class="bg-slate-50 border border-slate-200 rounded-lg p-4 w-full" style="background: var(--card); border-color: var(--border)">
              <div class="filter-grid">
                <div>
                  <label class="block text-sm font-medium mb-2" style="color: var(--muted)">Customer</label>
                  <select class="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent" style="background: var(--card); border-color: var(--border); color: var(--ink)">
                ...
                <div>
                  <label class="block text-sm font-medium mb-2" style="color: var(--muted)">Collection Status</label>
                  <select class="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent" style="background: var(--card); border-color: var(--border); color: var(--ink)">
                ...
                <div>
                  <label class="block text-sm font-medium mb-2" style="color: var(--muted)">From Date</label>
                  <input type="date" class="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent" style="background: var(--card); border-color: var(--border); color: var(--ink)">
                </div>
                <div>
                  <label class="block text-sm font-medium mb-2" style="color: var(--muted)">To Date</label>
                  <input type="date" class="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent" style="background: var(--card); border-color: var(--border); color: var(--ink)">
                </div>
                <div class="flex gap-2">
                  <button class="btn btn-brand px-6 flex-1">Apply Filters</button>
                  <button class="btn btn-soft px-6 flex-1">Reset</button>
                </div>
              </div>
            </div>
            ...
            <div class="table-responsive w-full">
              <table class="w-full text-sm">
                <thead class="bg-slate-50 border-b border-slate-200" style="background: var(--card); border-color: var(--border)">
                  <tr>
                    <th class="text-left px-4 py-3 font-semibold" style="color: var(--ink)">Date</th>
                    <th class="text-left px-4 py-3 font-semibold" style="color: var(--ink)">Reference</th>
                    <th class="text-left px-4 py-3 font-semibold" style="color: var(--ink)">Customer</th>
                    <th class="text-right px-4 py-3 font-semibold" style="color: var(--ink)">Amount</th>
                    <th class="text-center px-4 py-3 font-semibold" style="color: var(--ink)">Status</th>
                    <th class="text-center px-4 py-3 font-semibold" style="color: var(--ink)">Actions</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-slate-100" style="border-color: var(--border)">
                  <tr class="hover:bg-slate-50 transition-colors" style="border-color: var(--border)">
                    <td class="px-4 py-3" style="color: var(--ink)">2025-08-01</td>
                    <td class="px-4 py-3 font-mono font-medium" style="color: var(--ink)">COL-0001</td>
                    <td class="px-4 py-3" style="color: var(--muted)">Sample Customer</td>
                    <td class="px-4 py-3 text-right font-mono" style="color: var(--ink)">₱1,200.00</td>
                    <td class="px-4 py-3 text-center">
                      <span class="px-3 py-1 bg-green-100 text-green-800 rounded-full text-xs font-medium">Collected</span>
                    </td>
                    <td class="px-4 py-3 text-center">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
              <div class="bg-green-50 border border-green-200 rounded-lg p-4" style="background: rgba(6,78,59,.2); border-color: rgba(16,185,129,.3)">
                <div class="text-xs uppercase tracking-wide text-green-600 font-semibold" style="color: #6ee7b7">Total Collected</div>
                <div class="text-2xl font-bold text-green-800 mt-1" style="color: #10b981">₱15,750.00</div>
                <div class="text-xs text-green-600 mt-1" style="color: #34d399">This month</div>
              </div>
              <div class="bg-orange-50 border border-orange-200 rounded-lg p-4" style="background: rgba(194,65,12,.2); border-color: rgba(251,146,60,.3)">
                <div class="text-xs uppercase tracking-wide text-orange-600 font-semibold" style="color: #fb923c">Pending</div>
                <div class="text-2xl font-bold text-orange-800 mt-1" style="color: #f97316">₱3,200.00</div>
                <div class="text-xs text-orange-600 mt-1" style="color: #f97316">Awaiting collection</div>
              </div>
              <div class="bg-red-50 border border-red-200 rounded-lg p-4" style="background: rgba(239,68,68,.2); border-color: rgba(248,113,113,.3)">
                <div class="text-xs uppercase tracking-wide text-red-600 font-semibold" style="color: #fca5a5">Overdue</div>
                <div class="text-2xl font-bold text-red-800 mt-1" style="color: #f87171">₱1,800.00</div>
                <div class="text-xs text-red-600 mt-1" style="color: #f87171">Past due date</div>
              </div>
              <div class="bg-slate-50 border border-slate-200 rounded-lg p-4" style="background: rgba(51,65,85,.2); border-color: rgba(100,116,139,.3)">
                <div class="text-xs uppercase tracking-wide text-slate-600 font-semibold" style="color: #94a3b8">Collection Rate</div>
                <div class="text-2xl font-bold text-slate-800 mt-1" style="color: #e2e8f0">87%</div>
                <div class="text-xs text-slate-600 mt-1" style="color: #94a3b8">Success rate</div>
              </div>
            </div>
            
            <div class="text-sm text-center py-2" style="color: var(--muted)">
    ...
    function animateContentIn() {
      const cards = document.querySelectorAll('.collections-card, .card');
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      if(!location.hash){
        location.hash = '#Collections/col-pending';
      }
    ...
    function initNotificationSystem() {
      ...
      // Simulate real-time notifications for Collections
      simulateCollectionsNotifications();
    }
    ...
    // Simulate Collections-specific notifications
    function simulateCollectionsNotifications() {
      // Collection notifications (every 2 minutes)
      setInterval(() => {
        if (Math.random() < 0.25) { // 25% chance
          const collections = [
            'New collection assigned: Customer A - ₱' + Math.floor(Math.random() * 50000 + 10000).toLocaleString('en-US') + ' overdue',
            'Collection call completed: Customer B - Payment promise for next week',
            'Payment received: ₱' + Math.floor(Math.random() * 30000 + 5000).toLocaleString('en-US') + ' from Customer C',
            'Collection report generated: ₱' + Math.floor(Math.random() * 200000 + 100000).toLocaleString('en-US') + ' total outstanding'
          ];
          const randomCollection = collections[Math.floor(Math.random() * collections.length)];
          addNotificationToPanel('financial', randomCollection);
        }
      }, 120000);
      ...
      setInterval(() => {
        if (Math.random() < 0.2) { // 20% chance
          const systems = [
            'Collection reminder sent: 5 customers with overdue invoices',
            'Collection team performance: 85% success rate this month',
            'New collection strategy implemented: Early payment discounts',
            'Collection software updated: Enhanced customer tracking features'
          ];
          const randomSystem = systems[Math.floor(Math.random() * systems.length)];
          addNotificationToPanel('system', randomSystem);
        }
      }, 180000);
    }
